...
class Attn(nn.Module):
    ...
    def forward(self, out_state, history):
        ...
        attn_energies = Variable(torch.zeros(state_len, seq_len)).cpu()
...
class TrajPreAttnAvgLongUser(nn.Module):
    ...
    def forward(self, loc, tim, history_loc, history_tim, history_count, uid, target_len):
        ...
        count = 0
        loc_emb_history2 = Variable(torch.zeros(len(history_count), loc_emb_history.size()[-1])).cpu()
        tim_emb_history2 = Variable(torch.zeros(len(history_count), tim_emb_history.size()[-1])).cpu()
